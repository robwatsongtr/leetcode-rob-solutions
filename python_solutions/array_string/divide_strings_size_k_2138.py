...
consists of the next k characters of the string, and so on. Each element can be a 
art of exactly one group.
...
last group (if it exists) and concatenating all the groups in order, the resultant string should be s.
...
class Solution:
    def divideString(self, s: str, k: int, fill: str) -> List[str]:
        arr_len =  math.ceil(len(s) / k) * k # rounds up 
        arr = [fill] * arr_len
